...
describe("Offline Sync Idempotency", () => {
  it("should handle 50 concurrent requests for the same client_round_id safely", async () => {
    ...
    for (let i = 0; i < 50; i++) {
      promises.push(
        fetch(
          testEnv.supabaseUrl.replace("http://", "http://") +
            "/functions/v1/server/sync-offline-rounds",
          {
            method: "POST",
            headers: {
    ...
    const okCount = results.filter(
      (r) => r.results && r.results[0]?.status === "ok",
    ).length;
    const dupCount = results.filter(
      (r) => r.results && r.results[0]?.status === "duplicate",
    ).length;
    const blockedCount = results.filter(
      (r) => r.error && r.error.includes("Too many offline sync"),
    ).length;
