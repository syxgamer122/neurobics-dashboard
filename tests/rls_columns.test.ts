...
describe('Profiles Strict Column-level RLS', () => {
  it('should completely prevent authenticated user from direct UPDATE on profiles', async () => {
    ...
    await anonClient.auth.signInWithPassword({ email, password: 'Password123!' });

    // Try to update role
    const { error: roleErr } = await anonClient.from('profiles').update({ role: 'admin' }).eq('id', userId);
    expect(roleErr).not.toBeNull();
    expect(roleErr?.message).toMatch(/denied|permission|update/i);

    // Try to update total_xp
    const { error: xpErr } = await anonClient.from('profiles').update({ total_xp: 9999 }).eq('id', userId);
