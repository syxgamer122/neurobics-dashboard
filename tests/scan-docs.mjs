...
const rules = [
  ...
  { 
    ...
    excludeMatch: (m) => m.toLowerCase().includes("neurobics-ui") || m.toLowerCase().includes("neurobics.") || m.toLowerCase().includes("neurobics_") || m.toLowerCase().includes("@neurobics")
  }
];
  ...
  for (const rule of rules) {
    ...
    lines.forEach((line, i) => {
      ...
      if (match) {
        ...
        console.error(`[${rule.name}] ${path.basename(file)}:${i + 1}: ${line.trim()}`);
        errors++;
      }
    });
  }
