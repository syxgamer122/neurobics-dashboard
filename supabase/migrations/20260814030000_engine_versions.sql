-- Versioning Scoring, Telemetry & Anti-Cheat

-- 1. Add version columns to round_tickets (as it represents the raw submission)
alter table public.round_tickets add column if not exists telemetry_version integer;
alter table public.round_tickets add column if not exists scorer_version integer;
alter table public.round_tickets add column if not exists inspector_version integer;

-- 2. Add version columns to training_sessions (as it represents the processed analytics data)
alter table public.training_sessions add column if not exists telemetry_version integer;
alter table public.training_sessions add column if not exists scorer_version integer;
alter table public.training_sessions add column if not exists inspector_version integer;

-- 3. Replace submit_round_transaction to accept versions
...
revoke all on function public.submit_round_transaction(uuid, uuid, text, jsonb, integer, text, integer, integer, integer, integer) from public, anon, authenticated;
grant execute on function public.submit_round_transaction(uuid, uuid, text, jsonb, integer, text, integer, integer, integer, integer) to service_role;
