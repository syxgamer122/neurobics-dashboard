-- ═══════════════════════════════════════════════════════════════════════════
-- 20260824_mental_rotation_game.sql
-- Game mới: Mental Rotation 2D — hình dung xoay (Spatial + Speed).
-- ═══════════════════════════════════════════════════════════════════════════
...
alter table public.profiles
  add column if not exists mental_sessions integer not null default 0;

revoke update (mental_sessions) on public.profiles from authenticated, anon;
...
alter table public.round_tickets
  ...
  check (game in ('schulte','sudoku','stroop','reaction','memory','nback','math','gonogo','mental'));
...
alter table public.xp_events
  ...
  check (game in ('schulte','sudoku','stroop','memory','reaction','nback','math','gonogo','mental','quest','achievement'));

-- 3) submit_round_transaction: nhận 'mental' + tăng mental_sessions (giữ gonogo)
...
declare
  ...
  v_recent integer;
begin
  if p_game not in ('schulte','sudoku','stroop','reaction','memory','nback','math','gonogo','mental') then
...
-- 4) get_population_stats: cộng mental_sessions vào ngưỡng hiệu chuẩn
