-- ═══════════════════════════════════════════════════════════════════════════
-- 20260809_math_game.sql
-- Game mới: Math Sprint (toán nhanh) — logic + tốc độ.
-- ═══════════════════════════════════════════════════════════════════════════

-- 1) Cột đếm ván
alter table public.profiles
  add column if not exists math_sessions integer not null default 0;

revoke update (math_sessions) on public.profiles from authenticated, anon;
...
alter table public.round_tickets
  ...
  check (game in ('schulte','sudoku','stroop','reaction','memory','nback','math'));
...
alter table public.xp_events
  ...
  check (game in ('schulte','sudoku','stroop','memory','reaction','nback','math','quest','achievement'));

-- 3) submit_round_transaction: nhận 'math' + tăng math_sessions
-- (giữ nguyên phần chống gian lận / rate limit của 20260808)
...
-- 4) get_population_stats: cộng math_sessions vào ngưỡng hiệu chuẩn
-- (get_leaderboard returns setof profiles — thêm cột không đổi kiểu trả về,
--  nhưng DROP trước cho an toàn với mọi bản Postgres)
...
-- get_leaderboard: drop + recreate vì returns setof profiles (row type đổi khi thêm cột)
