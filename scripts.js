...
gamesScoring = gamesScoring.replace('Gửi request POST /server/submit-round với game, 	elemetry, và ingerprint.', 'Gửi request POST /server/submit-round với game, 	elemetry, và ingerprint. (Truyền thêm p_scorer_version bằng SCORER_VERSIONS[game]).');
gamesScoring = gamesScoring.replace('nhận request, đánh giá điểm dựa trên model chấm điểm (cập nhật động từ game-registry.ts)', 'nhận request, đánh giá điểm dựa trên model chấm điểm (cập nhật động từ game-registry.ts) và ghi nhận ating_model_version vào bảng profiles');
gamesScoring = gamesScoring.replace('compute_cognitive_index + view friend_leaderboard', 'compute_cognitive_index + view friend_leaderboard + view profiles_decayed (nay có trả thêm rating_model_version và flagged)');
...
let adr6 = fs.readFileSync('docs/adr/ADR-0006-append-only-audit-log.md', 'utf8');
adr6 += \
## Ngoại lệ
