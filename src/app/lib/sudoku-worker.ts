...
// Bundlers that support `?worker` (Vite) will compile this file as a worker entry.
...
const countSolutions = (grid: (number | null)[][], limit = 2): number => {
...
self.onmessage = (ev: MessageEvent<SudokuWorkerRequest>) => {
  ...
  const payload: SudokuWorkerResponse = { requestId, ...result };
