...
test.describe("Offline Sync", () => {
  test("app loads and renders even when sync endpoint fails", async ({
    page,
  }) => {
  ...
  test("offline queue key exists in localStorage after guest login", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("app does not crash when submit-round endpoint is down", async ({
    page,
  }) => {
