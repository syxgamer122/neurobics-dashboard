...
test.describe("Gameplay Flow", () => {
  ...
  test("can start and load a game (Schulte)", async ({ page }) => {
    ...
    await expect(
      page.locator("canvas, [data-testid='game-grid'], .game-container").first()
  ...
  test("can navigate between different games", async ({ page }) => {
    ...
    const backBtn = page.locator("button:has-text('Back'), button:has-text('Quay lại'), [aria-label='Back']").first();
    if (await backBtn.isVisible({ timeout: 2000 }).catch(() => false)) {
      await backBtn.click();
    } else {
      await page.goto("/");
      // Re-login if needed
      if (await page.locator("text=Guest Mode").isVisible({ timeout: 1000 }).catch(() => false)) {
        await page.click("text=Guest Mode");
        await page.fill("input[type='number']", "1990");
        await page.click("text=Continue");
      }
  ...
  test("all 12 game cards are visible in arena", async ({ page }) => {
    ...
    const gameNames = ["Schulte Table", "Sudoku", "Stroop Test", "Reaction Time", "Memory Matrix"];
