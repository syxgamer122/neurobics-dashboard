...
### Phân tích chi tiết nguyên nhân trong SQL:
Trong file migration `supabase/migrations/20260910000009_phase10_xp_inflation_quests.sql` (dòng 207-208), hàm `public.claim_quest(p_code text)` có câu lệnh:

```sql
-- LỖI CÚ PHÁP: 5 cột đích nhưng chỉ có 4 giá trị trong VALUES (...)
INSERT INTO public.xp_events(user_id, game, round_score, xp_awarded, source_key)
VALUES (v_user, 'quest', 0, v_xp);
```

- **Cột đích (5 cột)**: `user_id`, `game`, `round_score`, `xp_awarded`, `source_key`
- **Giá trị cung cấp (4 giá trị)**: `v_user`, `'quest'`, `0`, `v_xp` $ightarrow$ **Thiếu giá trị thứ 5 (`source_key`)**!
- PostgreSQL báo lỗi trực tiếp: `INSERT has more target columns than expressions` và trả về mã lỗi HTTP 400.

---

## 2. CÂU LỆNH SQL SỬA LỖI NGAY TRÊN SUPABASE DASHBOARD

Mở **Supabase Dashboard -> SQL Editor** của dự án và chạy đoạn script sau:

```sql
-- ==============================================================================
-- FIX RPC claim_quest: Đầy đủ 5 cột và 5 giá trị + Idempotent source_key
-- ==============================================================================
...
BEGIN
  ...
  -- 5 cột đích khớp chính xác 5 giá trị trong VALUES
  INSERT INTO public.xp_events(user_id, game, round_score, xp_awarded, source_key)
