...
- **Guest Account Recovery (ADR-0009)**: Guest "nâng cấp" thành User thông qua cơ chế **State Machine** tại endpoint `/server/upgrade-account`. Trạng thái chờ email sẽ lưu ở `upgrade_operations`, và quyền truy cập thực sự chỉ được cấp phát bằng Database Trigger sau khi xác thực email thành công. Việc client tự ý gọi `updateUser()` bị cấm hoàn toàn.
